    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    logger.LogInformation("User created: " + model.Username);

                    // Retrieve created user
                    user = await this.userManager.FindByNameAsync(user.UserName);

                    // Add new player if it's a player registration
                    if (model.IsPlayer)
                    {
                        ...
                        newPlayer = await this.playerRepository.InsertAsync(newPlayer);

                        // Update the new user with PlayerId
                        user.PlayerId = newPlayer.Id;
                        await this.userManager.UpdateAsync(user);

                        logger.LogInformation("Player created: " + newPlayer.FirstName + " " + newPlayer.LastName);
                    }
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                logger.LogInformation("User confirmed: " + user.UserName);
                return Ok();
            }
